...
class StandardRoIHead(BaseRoIHead):
    ...
    def bbox_loss(self, x: Tuple[Tensor],
        ...
        indicesP = []
        indicesN = []
        for idx, sampling in enumerate(sampling_results):
            # Calcule a posição inicial para esta imagem
            position = idx * sampling.avg_factor  # Multiplicando pelo tamanho do vetor de features por imagem

            # Calcule os índices de início e fim
            start_index = position + sampling.num_pos
            end_index = start_index + sampling.num_neg
            # Adicione os índices à lista
            indicesN.extend(range(start_index, end_index))

            # Calcule os índices de início e fim
            start_index = position
            end_index = start_index + sampling.num_pos
            
            # Adicione os índices à lista
            indicesP.extend(range(start_index, end_index))
        device = bbox_results['bbox_feats'].device
        
        neg_Embbeds = bbox_results['bbox_feats'][indicesN]
        pos_Embbeds = bbox_results['bbox_feats'][indicesP]
        bbox_loss_and_target['loss_bbox']['loss_clus'] = self.bbox_head.loss_feat_clusters(pos_Embbeds,neg_Embbeds)
        #with torch.no_grad():
